# AGENTS.md — ChurnShield Architecture Guide

This document provides an overview of the ChurnShield project structure for developers and AI agents.

## Project Overview

ChurnShield is a dashboard that showcases an ML churn-prediction model trained on the Kaggle Telco Customer Churn dataset. It visualises model metrics (accuracy, AUC-ROC, SMOTE recall lift) and exposes a live predictor form backed by a client-side logistic regression approximation that mirrors the deployed Flask REST API.

The original Python model (scikit-learn + SMOTE) is deployed separately on Render. This Netlify project is the frontend showcase and demo interface.
